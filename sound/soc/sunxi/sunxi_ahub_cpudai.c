...
no_karaook_handle:
	for (i = 0; i < 4; i++) {
		reg_val = (sunxi_ahub_read(SUNXI_AHUB_I2S_RXCONT(i)) >> 29) & 0x07;
		if ((reg_val == 1 && sunxi_ahub_cpudai->id == 2) ||
		    	(reg_val == 2 && sunxi_ahub_cpudai->id == 1) ||
		    	(reg_val == 4 && sunxi_ahub_cpudai->id == 0)) {
		
			sunxi_ahub_update_bits(SUNXI_AHUB_RST,
					(1<<(23 - i)), (1<<(23 - i)));
				sunxi_ahub_update_bits(SUNXI_AHUB_GAT,
					(1<<(23 - i)), (1<<(23 - i)));
				sunxi_ahub_update_bits(SUNXI_AHUB_I2S_CTL(i),
					(1<<I2S_CTL_TXEN), (1<<I2S_CTL_TXEN));
			break;
		}
	}
