		...
		struct snd_pcm_hw_params *params)
{
	...
	/* set system clk div */
	if (strcmp(codec_dai->name, "wm8960-hifi") == 0) {
		if (params_format(params) == SNDRV_PCM_FORMAT_S16_LE)
			clk_div = freq / params_rate(params) / 16 / params_channels(params);
		else if (params_format(params) == SNDRV_PCM_FORMAT_S24_LE)
			clk_div = freq / params_rate(params) / 24 / params_channels(params);
		else
			clk_div = freq / params_rate(params) / 32 / params_channels(params);
	} else {
	...
	if (strcmp(codec_dai->name, "wm8960-hifi") == 0) {
		ret = snd_soc_dai_set_pll(codec_dai, WM8960_SYSCLK_MCLK, 0, freq / 8, freq / 8);
		if (ret)
			return ret;

		ret = snd_soc_dai_set_sysclk(codec_dai, WM8960_SYSCLK_MCLK, freq / 8, 0);
	} else {
