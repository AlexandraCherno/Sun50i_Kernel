...
enum disp_dvi_hdmi   g_output_dvi_hdmi = DISP_HDMI;
#endif
...
#ifdef CONFIG_SUNXI_ANDROID
	mgrp->cfg->config.color_range = dev_config.range;
#endif
	mgrp->cfg->flag |= MANAGER_COLOR_SPACE_DIRTY;
	spin_unlock_irqrestore(&mgr_data_lock, flags);
...
#if 0
#ifndef CONFIG_SUNXI_ANDROID
		chn_num = de_feat_get_num_chns(mgr->disp);
		for (chn = 0; chn < chn_num; ++chn) {
			struct disp_layer_config_data *pdata = lyr_cfg;
			struct disp_layer_config_data *data_end = pdata + num_layers;

			for (; pdata != data_end; ++pdata) {
				if (pdata->config.channel != chn)
					continue;

				if (pdata->config.enable) {
					pdata->config.info.screen_win.width = mgrp->cfg->config.size.width;
					pdata->config.info.screen_win.height = mgrp->cfg->config.size.height;
				}
			}
		}
#endif
...
#ifndef CONFIG_SUNXI_ANDROID
	//使用HDMI转DVI 设置成DISP_DVI， 使用HDMI线 写DISP_HDMI
	dev_config.dvi_hdmi = g_output_dvi_hdmi;
#else
