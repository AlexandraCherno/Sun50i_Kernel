...
#ifndef CONFIG_SUNXI_ANDROID
		config.dvi_hdmi = ((g_output_dvi_hdmi > 2) || (g_output_dvi_hdmi < 0)) ? DISP_HDMI : g_output_dvi_hdmi;
#else
...
s32 bsp_disp_sync_with_hw(struct disp_bootloader_info boot_info)
{
	if ((boot_info.sync == 1)
	    && (boot_info.type != DISP_OUTPUT_TYPE_NONE)) {
		int num_screens = 0;
		int disp = boot_info.disp;
		int disp_dev = disp;
		enum disp_output_type type =
		    (enum disp_output_type)boot_info.type;
		enum disp_tv_mode mode =
			(enum disp_tv_mode)boot_info.mode;
		enum disp_csc_type format =
			(enum disp_csc_type)boot_info.format;
		enum disp_data_bits bits =
			(enum disp_data_bits)boot_info.bits;
		enum disp_color_space cs =
			(enum disp_color_space)boot_info.cs;
		enum disp_eotf eotf = (enum disp_eotf)boot_info.eotf;
		...
		config.eotf = eotf;
		config.cs = cs;
		config.dvi_hdmi = boot_info.dvi_hdmi;
		config.range = boot_info.range;
		config.scan = boot_info.scan;
		config.aspect_ratio = boot_info.aspect_ratio;
