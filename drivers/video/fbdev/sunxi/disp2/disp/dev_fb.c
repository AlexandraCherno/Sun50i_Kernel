...
struct fb_info_t {
	...
	struct disp_layer_config config1[SUNXI_FB_MAX];
};
...
#ifndef MODULE
static u32 disp_reserve_size0;
static u32 disp_reserve_base0;
static u32 disp_reserve_size1;
static u32 disp_reserve_base1;
static u32 disp_reserve_cnt;
int disp_reserve_mem(bool reserve)
{
	if (!disp_reserve_size0 || !disp_reserve_base0)
		return -EINVAL;

	if (disp_reserve_cnt == 2 && (!disp_reserve_size1 || !disp_reserve_base1))
		return -EINVAL;

	if (reserve){
		memblock_reserve(disp_reserve_base0, disp_reserve_size0);
		if (disp_reserve_cnt == 2)
			memblock_reserve(disp_reserve_base1, disp_reserve_size1);
	} else {
		memblock_free(disp_reserve_base0, disp_reserve_size0);
		if (disp_reserve_cnt == 2)
			memblock_free(disp_reserve_base1, disp_reserve_size1);
	}
	return 0;
}
...
static int __init early_disp_reserve(char *str)
{
	...
	get_options(str, 5, temp);

	disp_reserve_size0 = temp[1];
	disp_reserve_base0 = temp[2];
	disp_reserve_cnt = 1;

	if (temp[0] == 4) {
		disp_reserve_cnt = 2;
		disp_reserve_size1 = temp[3];
		disp_reserve_base1 = temp[4];
	}

	if ((temp[0] == 2 && disp_reserve_size0 <= 0) || (temp[0] == 4 && disp_reserve_size1 <= 0))
		return -EINVAL;

	/*arch_mem_end = memblock_end_of_DRAM();*/
	disp_reserve_mem(true);
	pr_info("disp reserve base 0x%x ,size 0x%x\n",
					disp_reserve_base0, disp_reserve_size0);

	if (disp_reserve_cnt == 2)
		pr_info("disp reserve base 0x%x ,size 0x%x\n",
					disp_reserve_base1, disp_reserve_size1);
...
#ifdef CONFIG_DISP2_SUNXI_MUTI_SCREEN
static int fb_wait_for_vsync_by_screen(u32 screen_id)
{
	unsigned long count;
	int ret;
	...
	count = g_fbi.wait_count[screen_id];
	ret =
	    wait_event_interruptible_timeout(g_fbi.wait[screen_id],
					     count !=
					     g_fbi.
					     wait_count[screen_id],
					     msecs_to_jiffies
					     (50));
	if (ret == 0) {
		__inf("timeout\n");
...
static int fb_wait_for_vsync(struct fb_info *info)
{
	...
	num_screens = bsp_disp_feat_get_num_screens();
		switch (g_fbi.fb_para[fb_id].fb_mode) {
			case DISP_INIT_MODE_SCREEN0: {
				screen_id = 0;//bind all fb to screen0
				ret = fb_wait_for_vsync_by_screen(screen_id);
				break;
			}
			...
			case FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS: {
				if (fb_id == 0) {//bind fb0 to all screen.
...
static int sunxi_fb_pan_display_by_screen(struct fb_var_screeninfo *var,u32 fb_id, u32 screen_id)
{
	u32 buffer_num = 1;
	u32 y_offset = 0;
	...
	if(FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS == g_fbi.fb_para[fb_id].fb_mode)	{
		if(screen_id == 0){
			config = &g_fbi.config[fb_id];
		}else {
			config = &g_fbi.config1[fb_id];
		}
	} else
		config = &g_fbi.config[fb_id];
				...
				struct fb_info *info)
{
	...
	switch (g_fbi.fb_para[fb_id].fb_mode) {
		case DISP_INIT_MODE_SCREEN0: {
			screen_id = 0;//bind all fb to screen0
			sunxi_fb_pan_display_by_screen(var,fb_id,screen_id);
			break;
		}
		case DISP_INIT_MODE_SCREEN1: {
			screen_id = 1;//bind all fb to screen1
...
static int sunxi_fb_blank_by_screen(int blank_mode,u32 fb_id, u32 screen_id)
{
	s32 chan = g_fbi.layer_hdl[fb_id][0];
	s32 layer_id = g_fbi.layer_hdl[fb_id][1];
	struct disp_layer_config* config = NULL;
...
static int sunxi_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	switch (g_fbi.fb_para[fb_id].fb_mode) {
		case DISP_INIT_MODE_SCREEN0: {
			screen_id = 0;//bind all fb to screen0
			sunxi_fb_blank_by_screen(blank_mode,fb_id,screen_id);
			break;
		}
		case DISP_INIT_MODE_SCREEN1: {
			screen_id = 1;//bind all fb to screen1
...
static int __fb_wait_for_vsync(struct fb_info *info)
{
	unsigned long count;
	u32 sel = 0;
	int ret;
...
static int __sunxi_fb_pan_display(struct fb_var_screeninfo *var,
				struct fb_info *info)
{
	u32 sel = 0;
	u32 num_screens;
...
static int __sunxi_fb_blank(int blank_mode, struct fb_info *info)
{
	u32 sel = 0;
	u32 num_screens;
...
static int Fb_copy_boot_fb(u32 sel, struct fb_info *info)
{
	...
	if (sel == 0)
		...
		boot_fb_str = (char *)disp_boot_para_parse_str("boot_fb1");
#else
	boot_fb_str = (char *)disp_boot_para_parse_str("boot_fb0");
#endif
	if (boot_fb_str != NULL) {
...
#ifdef CONFIG_DISP2_SUNXI_MUTI_SCREEN
static s32 display_fb_release_by_screen(u32 screen_id,u8 fb_id)
{
	struct disp_manager *mgr = NULL;
	struct disp_layer_config config;
...
static s32 display_fb_release(u32 fb_id)
{
	...
	if (g_fbi.fb_enable[fb_id]) {
		...
		switch (g_fbi.fb_para[fb_id].fb_mode) {
			case DISP_INIT_MODE_SCREEN0: {
				screen_id = 0;//bind all fb to screen0
				display_fb_release_by_screen(screen_id,fb_id);
				break;
			}
			case DISP_INIT_MODE_SCREEN1: {
				screen_id = 1;//bind all fb to screen1
...
s32 Display_set_fb_timming_by_screen(u32 sel,u8 fb_id)
{
	struct disp_video_timings tt;
	struct disp_manager *mgr = g_disp_drv.mgr[sel];
	...
	for (fb_id = 0; fb_id < SUNXI_FB_MAX; fb_id++) {
		if (g_fbi.fb_enable[fb_id]) {
			switch (g_fbi.fb_para[fb_id].fb_mode) {
				case DISP_INIT_MODE_SCREEN0: {
					screen_id = 0;//bind all fb to screen0
					Display_set_fb_timming_by_screen(screen_id,fb_id);
					break;
				}
				case DISP_INIT_MODE_SCREEN1: {
					screen_id = 1;//bind all fb to screen1
...
static int init_fb_var(u32 fb_num)
{
	int i = fb_num;
...
static int config_fb_create_info_by_screen(struct disp_fb_create_info *fb_para, u32 screen_id)
{
	int ret = 0;
	u32 disp_mode = 0;
	...
	if (0 == g_disp_drv.disp_init.b_init) {
		return -1;
	}

	disp_mode = g_disp_drv.disp_init.disp_mode;
	disp_init = &g_disp_drv.disp_init;

	fb_para->buffer_num = g_disp_drv.disp_init.buffer_num[screen_id];

	fb_para->output_width = bsp_disp_get_screen_width_from_output_type
	    (screen_id, disp_init->output_type[screen_id], disp_init->output_mode[screen_id]);
	fb_para->output_height = bsp_disp_get_screen_height_from_output_type
	    (screen_id, disp_init->output_type[screen_id], disp_init->output_mode[screen_id]);
	...
	__wrn("%s:fb width = %d, height = %d\n", __func__,
			fb_para->output_width, fb_para->output_height);
	fb_para->width = fb_para->output_width;
	fb_para->height = fb_para->output_height;
#endif
	__wrn("%s buf:%d,fb_mode:%d,[%dx%d],output[%dx%d]\n", __func__, fb_para->buffer_num, fb_para->fb_mode,
			fb_para->width, fb_para->height, fb_para->output_width, fb_para->output_height);
	return ret;
}
static int config_fb_by_screen(u32 fb_id, u32 screen_id)
{
	int ret;
	struct disp_init_para *disp_init = &g_disp_drv.disp_init;

	int fb_width = disp_init->fb_width[screen_id];
	int fb_height = disp_init->fb_height[screen_id];
	int scr_width = bsp_disp_get_screen_width_from_output_type
							(screen_id, g_disp_drv.disp_init.output_type[screen_id],
										g_disp_drv.disp_init.output_mode[screen_id]);
	int scr_height = bsp_disp_get_screen_height_from_output_type
							(screen_id, g_disp_drv.disp_init.output_type[screen_id],
										g_disp_drv.disp_init.output_mode[screen_id]);
	...
	g_fbi.fbinfo[fb_id]->var.xoffset = 0;
	g_fbi.fbinfo[fb_id]->var.yoffset = 0;
	g_fbi.fbinfo[fb_id]->var.xres = fb_width;
	g_fbi.fbinfo[fb_id]->var.yres = fb_height;
	g_fbi.fbinfo[fb_id]->var.xres_virtual = fb_width;
	g_fbi.fbinfo[fb_id]->fix.line_length = (fb_width * g_fbi.fbinfo[fb_id]->var.bits_per_pixel) >> 3;
	g_fbi.fbinfo[fb_id]->fix.smem_len = g_fbi.fbinfo[fb_id]->fix.line_length * fb_height * g_disp_drv.disp_init.buffer_num[screen_id];;
	if (g_fbi.fbinfo[fb_id]->fix.line_length != 0)
		g_fbi.fbinfo[fb_id]->var.yres_virtual = g_fbi.fbinfo[fb_id]->fix.smem_len / g_fbi.fbinfo[fb_id]->fix.line_length;

	g_fbi.fbinfo[fb_id]->var.width  = scr_width;
	g_fbi.fbinfo[fb_id]->var.height = scr_height;
	__wrn("%s fb[%d] res[%dx%d],size in mm[%dx%d]\n", __func__, fb_id, fb_width, fb_height, scr_width, scr_height);

	disp_fb_to_var(disp_init->format[screen_id],
		       &(g_fbi.fbinfo[fb_id]->var));//根据disp_init_para设置fb图像格式参数

	ret = fb_map_video_memory(g_fbi.fbinfo[fb_id]);
	return ret;

}
static int bind_fb_to_screen(u32 fb_id, u32 screen_id)
{
	int ret = 0;
	struct disp_layer_config config;
	u32 xres, yres;
	struct fb_info *fb_info_tmp = g_fbi.fbinfo[fb_id];
	u32 x_offset = 0;
	u32 y_offset = 0;
	struct disp_video_timings tt;
	struct disp_manager *mgr = NULL;

	__wrn("fb-->scr %d->%d,fb_mode=%d,addr=0x%lx\n", fb_id, screen_id,
				g_fbi.fb_para[fb_id].fb_mode, fb_info_tmp->fix.smem_start);
	...
	if (fb_id < 3) {
		...
		g_fbi.layer_hdl[fb_id][1] = 0;
	}	else if (fb_id < SUNXI_FB_MAX) {
		g_fbi.layer_hdl[fb_id][0] = 3;
		g_fbi.layer_hdl[fb_id][1] = 0;
	}

#ifndef CONFIG_SUNXI_ANDROID
	if (g_disp_drv.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN) {
		if (screen_id == 1) { 
			g_fbi.layer_hdl[fb_id][0] = fb_id;
		}
	} 
#endif
#endif

	ret = config_fb_create_info_by_screen(&g_fbi.fb_para[fb_id], screen_id);
	if (ret < 0) {
		return ret;
	}
	...
	xres = g_fbi.fb_para[fb_id].width;
	yres = g_fbi.fb_para[fb_id].height;
	if ((xres == 0) || (yres == 0) || (g_fbi.fbinfo[fb_id]->var.bits_per_pixel == 0)) {
		__wrn("invalid paras xres(%d), yres(%d) bpp(%d)\n", xres, yres, g_fbi.fbinfo[fb_id]->var.bits_per_pixel);
		return -1;
	}

	mgr = g_disp_drv.mgr[screen_id];//根据屏幕mgr信息设置FB参数
	...
	config.enable = 1;
	if ((0 == fb_id) || ((1 == fb_id) &&
		(FB_MODE_DUAL_DIFF_SCREEN == g_fbi.fb_para[fb_id].fb_mode))) {
	    Fb_map_kernel_logo(screen_id, fb_info_tmp);
	}
	if (g_disp_drv.para.boot_info.sync == 1) {
		if ((screen_id == g_disp_drv.para.boot_info.disp) &&
		    (g_disp_drv.para.boot_info.type != DISP_OUTPUT_TYPE_NONE)) {
			bsp_disp_get_display_size(screen_id, &g_fbi.fb_para[fb_id].output_width,
			      &g_fbi.fb_para[fb_id].output_height);
#ifndef CONFIG_SUNXI_ANDROID
			if ((fb_id >= 1) && (DISP_INIT_MODE_SCREEN1 == g_fbi.fb_para[fb_id].fb_mode)) {
					Fb_map_kernel_logo(screen_id, fb_info_tmp);
			}
#endif
		}
	...
	config.info.fb.crop.x = ((long long)x_offset) << 32;
	config.info.fb.crop.y = ((long long)y_offset) << 32;
	config.info.fb.crop.width = ((long long)xres) << 32;
	config.info.fb.crop.height = ((long long)yres) << 32;
	config.info.screen_win.x = 0;
	config.info.screen_win.y = 0;
	var_to_disp_fb(&(config.info.fb), &(fb_info_tmp->var), &(fb_info_tmp->fix));
	config.info.fb.addr[0] = (unsigned long long)fb_info_tmp->fix.smem_start;
	config.info.fb.addr[1] = 0;
	config.info.fb.addr[2] = 0;
	config.info.fb.flags = DISP_BF_NORMAL;
	config.info.fb.scan = DISP_SCAN_PROGRESSIVE;
	config.info.fb.size[0].width = g_fbi.fb_para[fb_id].width;
	config.info.fb.size[0].height = g_fbi.fb_para[fb_id].height;
	config.info.fb.size[1].width = g_fbi.fb_para[fb_id].width;
	config.info.fb.size[1].height = g_fbi.fb_para[fb_id].height;
	config.info.fb.size[2].width = g_fbi.fb_para[fb_id].width;
	config.info.fb.size[2].height = g_fbi.fb_para[fb_id].height;
	config.info.fb.color_space = DISP_BT601;

	__wrn("%s fb[%d],size[%dx%d],crop(%lld:%lld)[%lldx%lld],screen[%dx%d]\n", __func__,fb_id,
			config.info.fb.size[0].width, config.info.fb.size[0].height,
			config.info.fb.crop.x, config.info.fb.crop.y, config.info.fb.crop.width >> 32, config.info.fb.crop.height >> 32,
			config.info.screen_win.width, config.info.screen_win.height);

#if defined(CONFIG_ION_SUNXI)
	disp_ion_flush_cache((void *)g_fbi.mem[fb_info_tmp->node]->vaddr, g_fbi.mem[fb_info_tmp->node]->size);
#endif
#if (!defined(CONFIG_EINK_PANEL_USED)) && (!defined(CONFIG_EINK200_SUNXI))
	if (mgr && mgr->set_layer_config) {
		ret = mgr->set_layer_config(mgr, &config, 1);
		if (ret != 0) {
			__wrn("set_layer_config failed.");
			return ret;
		}
	}
#endif
	...
	if(FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS == g_fbi.fb_para[fb_id].fb_mode)	{
		if(screen_id == 0){
			...
			memcpy(&g_fbi.config1[fb_id], &config, sizeof(struct disp_layer_config));
		}			
	}else{
		memcpy(&g_fbi.config[fb_id], &config, sizeof(struct disp_layer_config));
	}
	...
	for (i = 0; i < SUNXI_FB_MAX; i++) {
		...
		switch (disp_mode) {
			...
			case DISP_INIT_MODE_TWO_DIFF_SCREEN: {
				...
				config_fb_by_screen(i, screen_id);
				bind_fb_to_screen(i, screen_id);		//bind all fb to screen0
				break;
			}
			case FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS: {
		...
		if (ret) {//bind screen failed.
			__wrn("bind_fb_to_screen failed.\n");
			break;
		}

		g_fbi.fb_enable[i] = 1;
		g_fbi.fb_mode[i] = g_fbi.fb_para[i].fb_mode;

#if defined(CONFIG_DISP2_SUNXI_BOOT_COLORBAR)
		fb_draw_colorbar((char *__force)g_fbi.fbinfo[i]->
				 screen_base, g_fbi.fb_para[i].width,
				 g_fbi.fb_para[i].height * g_fbi.fb_para[i].buffer_num,
				 &(g_fbi.fbinfo[i]->var));
#endif
	}
