...
#ifndef CONFIG_SUNXI_ANDROID
int g_boot_hdmi_status = 1; // on hdmi
#endif

#ifndef CONFIG_SUNXI_ANDROID
int g_tv_hdmi_enabled = 0;
#endif
...
#ifndef CONFIG_SUNXI_ANDROID
static int lcd_devices_iommu_status = 0;
#endif
...
#ifndef CONFIG_SUNXI_ANDROID
	__wrn("===YTR==init_para->disp_mode=%d,func=%s\n",init_para->disp_mode,__func__);
#endif
...
#ifndef CONFIG_SUNXI_ANDROID
		if ((init_para->disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN) 
				|| (init_para->disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS)) {
			g_output_dvi_hdmi = init_para->output_dvi_hdmi[1]; //init_para->output_type[1];
		} else {
			g_output_dvi_hdmi = init_para->output_dvi_hdmi[0];
		}
#endif

#ifndef CONFIG_SUNXI_ANDROID
		if (of_property_read_u32(np, "tv_hdmi_enabled", &g_tv_hdmi_enabled) < 0) {
			g_tv_hdmi_enabled = 0;	
		}
#endif
...
static void start_work(struct work_struct *work)
{
	...
	if (g_disp_drv.para.boot_info.sync == 0) {
...
#ifndef CONFIG_SUNXI_ANDROID
			int tv_registered = bsp_disp_get_tv_registered();

			__inf("sel=%d, output_type=%d, lcd_reg=%d,hdmi_reg=%d,tv_registered=%d\n",
			     screen_id, output_type, lcd_registered,
			     hdmi_registered, tv_registered);
#endif
...
#ifndef CONFIG_SUNXI_ANDROID
				&& (screen_id == 1)) ||
				(disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN) ||
				(disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS)) {
#else
    				&& (screen_id == 1))) {
#endif
				if (output_type == DISP_OUTPUT_TYPE_LCD) {
					...
					if (hdmi_registered &&
						...
						suspend_output_type[screen_id] =
						    output_type;
#ifndef CONFIG_SUNXI_ANDROID
						if (lcd_devices_iommu_status == 1) {
							sunxi_enable_device_iommu(DE_MASTOR_ID, true);
							lcd_devices_iommu_status = 2;

							disp_device_set_config(
									&g_disp_drv.disp_init, 0);
							suspend_output_type[0] = DISP_OUTPUT_TYPE_LCD;
						}
#endif
					}
#ifndef CONFIG_SUNXI_ANDROID
...
#ifndef CONFIG_SUNXI_ANDROID
		if (g_disp_drv.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS ||
			g_disp_drv.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN ||
			g_disp_drv.disp_init.disp_mode == DISP_INIT_MODE_SCREEN1) {
				
		    if (!(bsp_disp_get_hdmi_registered() || bsp_disp_get_tv_registered()))
			    return;
		}

		struct disp_layer_config bak_config0;
		struct disp_layer_config bak_config1;
		struct disp_layer_config bak_config2;
		struct disp_layer_config config;
		struct disp_manager *mgr0 = disp_get_layer_manager(0);
		struct disp_manager *mgr1 = disp_get_layer_manager(1);
		// close layer
		if (g_disp_drv.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN) {
			// 0 3 0
			config.channel = 3;
			config.layer_id = 0;
			mgr0->get_layer_config(mgr0, &config, 1);
			memcpy(&bak_config0, &config, sizeof(struct disp_layer_config));
			config.enable = 0;
			mgr0->set_layer_config(mgr0, &config, 1);
			// 1 1 0
			config.channel = 1;
			config.layer_id = 0;
			mgr1->get_layer_config(mgr1, &config, 1);
			memcpy(&bak_config1, &config, sizeof(struct disp_layer_config));
			config.enable = 0;
			mgr1->set_layer_config(mgr1, &config, 1);

			mgr1->apply(mgr1);
			mgr1->update_regs(mgr1);
			mgr1->sync(mgr1, true);
			msleep(300);//wait for apply
		} else if (g_disp_drv.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS) {
			// 0 2 0
			config.channel = 2;
			config.layer_id = 0;
			mgr0->get_layer_config(mgr0, &config, 1);
			...
			config.layer_id = 0;
			mgr1->get_layer_config(mgr1, &config, 1);
			memcpy(&bak_config2, &config, sizeof(struct disp_layer_config));
			config.enable = 0;
			mgr1->set_layer_config(mgr1, &config, 1);
		...
		if (!g_tv_hdmi_enabled) {
			if (bsp_disp_get_output_type(g_disp_drv.para.boot_info.disp) !=
					g_disp_drv.para.boot_info.type) {
				bsp_disp_sync_with_hw(g_disp_drv.para.boot_info);
				suspend_output_type[g_disp_drv.para.boot_info.disp] =
					g_disp_drv.para.boot_info.type;
			}
		} else {
			// screen0
			if (bsp_disp_get_tv_registered() & (bsp_disp_get_output_type(0) != g_disp_drv.disp_init.output_type[0])) {
				bsp_disp_sync_with_hw(g_disp_drv.para.boot_info);
				suspend_output_type[g_disp_drv.para.boot_info.disp] =
					g_disp_drv.para.boot_info.type;
			} 
		...
		if (g_disp_drv.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS ||
			...
			boot_info.sync = 1;
			boot_info.type = g_disp_drv.disp_init.output_type[1];
			boot_info.disp = 1;
			boot_info.mode = g_disp_drv.disp_init.output_mode[1];
			boot_info.format = g_disp_drv.disp_init.output_format[1];
			boot_info.bits = g_disp_drv.disp_init.output_bits[1];
			boot_info.eotf = g_disp_drv.disp_init.output_eotf[1];
			boot_info.cs = g_disp_drv.disp_init.output_cs[1];
			boot_info.dvi_hdmi = g_disp_drv.disp_init.output_dvi_hdmi[1];
			boot_info.range = g_disp_drv.disp_init.output_range[1];
			boot_info.scan = g_disp_drv.disp_init.output_scan[1];
			boot_info.aspect_ratio = g_disp_drv.disp_init.output_aspect_ratio[1];

			if (bsp_disp_get_output_type(boot_info.disp) != boot_info.type) {
				bsp_disp_sync_with_hw(boot_info);
				suspend_output_type[boot_info.disp] = boot_info.type;
			}
		}
				
		// recover layer
		if (g_disp_drv.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN) {
			bak_config0.enable = 1;	// 0
			bak_config1.enable = 1;
			mgr0->set_layer_config(mgr0, &bak_config0, 1);
			mgr1->set_layer_config(mgr1, &bak_config1, 1);
		} else if (g_disp_drv.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS) {
			bak_config0.enable = 1;
			bak_config1.enable = 1;
			bak_config2.enable = 1;
			mgr0->set_layer_config(mgr0, &bak_config0, 1);
			mgr0->set_layer_config(mgr0, &bak_config1, 1);

			mgr1->set_layer_config(mgr1, &bak_config2, 1);
		} else {
			bak_config0.enable = 1;
			bak_config1.enable = 1;
			mgr0->set_layer_config(mgr0, &bak_config0, 1);
...
static s32 disp_init(struct platform_device *pdev)
{
	...
	if (para->boot_info.sync == 1) {
		__wrn("smooth display disp_mode %d screen:%d type:%d mode:%d\n", g_disp_drv.disp_init.disp_mode, para->boot_info.disp,
		...
		if (g_disp_drv.disp_init.disp_mode ==  DISP_INIT_MODE_SCREEN0) {
			lcd_devices_iommu_status = 2;
		} else {
			lcd_devices_iommu_status = 1;
		}
#else
		sunxi_enable_device_iommu(DE_MASTOR_ID, true);
#endif
	...
	case DISP_LCD_GET_BRIGHTNESS:
		{
			if (dispdev && dispdev->get_bright) {
				...
				if (copy_to_user((void __user *)ubuffer[1], &ret,
			     		sizeof(ret))) {
					__wrn("copy_from_user fail\n");
					return -EFAULT;
				}
