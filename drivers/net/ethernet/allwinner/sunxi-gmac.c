...
static char mac0_str[] = GETH_MAC_ADDRESS;
static char mac1_str[] = GETH_MAC_ADDRESS;
//module_param((char *)mac0_str, charp, S_IRUGO | S_IWUSR);
//MODULE_PARM_DESC(mac0_str, "MAC Address String.(xx:xx:xx:xx:xx:xx)");
...
#ifndef MODULE
static int __init set_mac0_addr(char *str)
{
	char *p = str;

	if (str && strlen(str))
		memcpy(mac0_str, p, 18);

	return 0;
}
__setup("mac0_addr=", set_mac0_addr);
