		     ...
		     struct sensor_list *sc)
{
	int ret;
	int gnum;	
	gnum = of_get_named_gpio_flags(np, name, 0, (enum of_gpio_flags *)&sc->gpio[RESET]);
	if (!gpio_is_valid(gnum)) {
		sc->gpio[RESET].gpio = GPIO_INDEX_INVALID;
		printk("fetch %s from device_tree failed\n", name);
		return -EINVAL;
	} else {
		ret = gpio_request(gnum, name);                                                                              
		if (ret == 0) {
			sc->gpio[RESET].io_ext = gnum;
		} else {
			sc->gpio[RESET].gpio = GPIO_INDEX_INVALID;
		}
	} 
		    ...
		    struct sensor_list *sc)
{
	...
	int ret;
	int gnum;	
	gnum = of_get_named_gpio_flags(np, name, 0, (enum of_gpio_flags *)&sc->gpio[PWDN]);
	if (!gpio_is_valid(gnum)) {
		sc->gpio[PWDN].gpio = GPIO_INDEX_INVALID;
		printk("fetch %s from device_tree failed\n", name);
		return -EINVAL;
	} else {
		ret = gpio_request(gnum, name);                                                                              
		if(ret == 0) {
			sc->gpio[PWDN].io_ext = gnum;
		} else {
			sc->gpio[PWDN].gpio = GPIO_INDEX_INVALID;
		}
	} 
