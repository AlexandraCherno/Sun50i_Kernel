...
{
	printk("===YTR==sunxi_efuse_ioctl: =%s\n", __func__);
		...
		unsigned long ioctl_param)
{
...
#if 1
	printk("===YTR==sunxi_efuse_ioctl: ioctl_num=%d\n", ioctl_num);
	printk("nfcr = %p\n", nfcr);
	printk("name = %s\n", nfcr->key_store.name);
	printk("len = %d\n", nfcr->key_store.len);
#endif
	...
	case SUNXI_EFUSE_READ:
		...
		err = nfcr->ret;
		if (!err) {
			if (copy_to_user((void __user *)nfcr->key_store.key_data, nfcr->temp_data, nfcr->key_store.len)) {
				err = -EFAULT;
				pr_err("copy_to_user: err:%d\n", err);
				goto _out;
			}
			if (copy_to_user((void __user *)ioctl_param, &nfcr->key_store, sizeof(nfcr->key_store))) {
				err = -EFAULT;
				pr_err("copy_to_user: err:%d\n", err);
				goto _out;
			}
...
static void sunxi_efuse_work(struct work_struct *data)
{
	...
	switch (fcpt->cmd) {
	case SUNXI_EFUSE_READ:
		fcpt->key_store.len = arm_svc_efuse_read(virt_to_phys((const volatile void *)fcpt->key_store.name),
				virt_to_phys((const volatile void *)fcpt->temp_data));
		fcpt->ret = (fcpt->key_store.len > 0 ? 0 : -1);
	break;
	case SUNXI_EFUSE_WRITE:
