...
static int panel_rel_width = 1024;
static int panel_rel_height = 600;
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
	...
	input_report_abs(ts->input_dev, ABS_MT_POSITION_X, (ts->abs_x_max - x) * panel_rel_width / ts->abs_x_max);
	input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, (ts->abs_y_max - y) * panel_rel_height / ts->abs_y_max);
	input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, w);
	input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, w);
	input_sync(ts->input_dev);
#else
	input_report_key(ts->input_dev, BTN_TOUCH, 1);
	input_report_abs(ts->input_dev, ABS_X, (ts->abs_x_max - x) * panel_rel_width / ts->abs_x_max);
	input_report_abs(ts->input_dev, ABS_Y, (ts->abs_y_max - y) * panel_rel_height / ts->abs_y_max);
	//input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, w);
	//input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, w);
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
	...
	input_set_abs_params(ts->pen_dev, ABS_X, 0, panel_rel_width, 0, 0);
	input_set_abs_params(ts->pen_dev, ABS_Y, 0, panel_rel_height, 0, 0);
...
static void gtp_pen_down(s32 x, s32 y, s32 w, s32 id)
{
	...
	input_report_abs(ts->pen_dev, ABS_MT_POSITION_X, (ts->abs_x_max - x) * panel_rel_width / ts->abs_x_max);
	input_report_abs(ts->pen_dev, ABS_MT_POSITION_Y, (ts->abs_y_max - y) * panel_rel_height / ts->abs_y_max);
	...
	input_report_abs(ts->pen_dev, ABS_X, (ts->abs_x_max - x) * panel_rel_width / ts->abs_x_max);
	input_report_abs(ts->pen_dev, ABS_Y, (ts->abs_y_max - y) * panel_rel_height / ts->abs_y_max);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	if (!np) {
		dev_err(&ts->client->dev, "missing device tree data\n");
		return -EINVAL;
	}
	if (!of_property_read_u32(np, "goodix,panel-width", &val32)) {
		if (val32 == 800) {
			panel_rel_width = 800;
			panel_rel_height = 480;
		}
	}
...
#if GTP_ICS_SLOT_REPORT
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, panel_rel_width, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, panel_rel_height, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_WIDTH_MAJOR, 0, 255, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0, 255, 0, 0);
#else
	input_set_abs_params(ts->input_dev, ABS_X, 0, panel_rel_width, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_Y, 0, panel_rel_height, 0, 0);
#endif
#if GTP_ICS_SLOT_REPORT
